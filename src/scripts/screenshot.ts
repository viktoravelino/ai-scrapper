...
export async function screenshotComponents(url: string, target: string, selectors: string[]) {
    console.log({
        url,
        target,
        selectors,
    ...
    for (let i = 0; i < filtered.length; i++) {
        ...
        const classes = (await element.evaluate((el) => el.classList.toString()))?.trim();
